...
def edit_lesson(request, course_id, lesson_id):
    ...
    if request.method == 'POST':
        ...
        if lesson_form.is_valid():  # Chỉ cần kiểm tra lesson_form là đủ
            lesson = lesson_form.save(commit=False)
            lesson.course = course
            lesson.save()

            # Lưu resource nếu có file
            resource_file = request.FILES.get('resource_file')
            if resource_file:
                resource = LessonResource(
                    lesson=lesson,
                    resource_type=resource_form.cleaned_data.get('resource_type'),  # Dùng get để tránh lỗi nếu không có
                    resource_file=resource_file,
                )
                resource.save()
        ...
        resource_form = ResourceForm()  # Có thể điền sẵn resource nếu cần

    context = {
        'lesson_form': lesson_form,
        'resource_form': resource_form,
        'course': course,
        'lesson': lesson,  # Truyền đối tượng lesson để điền giá trị vào form
    }
    return render(request, 'lessons/add_lesson.html', context)
